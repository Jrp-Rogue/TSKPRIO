...
if choix == "Dashboard":
    st.subheader("📊 Vue d'ensemble des Tâches")
    st.write(f"Nombre total de tâches : {len(st.session_state.taches)}")
    if st.session_state.taches:
        st.write("Tâches récentes :")
        for tache in st.session_state.taches[-5:]:  # Affiche les 5 dernières tâches
            st.write(f"- {tache['nom']} (Urgence: {tache['urgence']}, Importance: {tache['importance']})")
    else:
        st.write("Aucune tâche à afficher.")

# 📌 Ajouter une tâche
elif choix == "Ajouter une tâche":
